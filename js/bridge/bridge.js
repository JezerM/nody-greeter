...
class Greeter {
    ...
    _connect_signals() {
        LightDMGreeter.connect("authentication-complete", () => { this._emit_signal("authentication-complete"); });
        LightDMGreeter.connect("autologin-timer-expired", () => { this._emit_signal("autologin-timer-expired"); });
        LightDMGreeter.connect("show-message", () => { this._emit_signal("show-message"); });
        LightDMGreeter.connect("show-prompt", () => { this._emit_signal("show-prompt"); });
        LightDMGreeter.connect("idle", () => { this._emit_signal("idle"); });
        LightDMGreeter.connect("reset", () => { this._emit_signal("reset"); });
    }
    _emit_signal(signal) {
...
class ThemeUtils {
    ...
    bind_this(context) {
        ...
        function not_excluded(_method, _context) {
            let is_excluded = excluded_methods.findIndex(excluded_method => _method === excluded_method) > -1, is_method = 'function' === typeof _context[_method];
...
function handler(ldm, ev, ...args) {
    ...
    else if (str.includes("LightDMLayout")) {
        if (Array.isArray(value)) {
    ...
    else if (str.includes("LightDMSession")) {
        if (Array.isArray(value)) {
    ...
    else if (str.includes("LightDMUser")) {
        if (Array.isArray(value)) {
